#ifndef _COMMON_H_
#define _COMMON_H_

#include <stdint.h>

#define KYBER_N  256
#define KYBER_Q  3329

typedef struct {
  uint32_t grid_dim[1];
  uint32_t block_dim[1];
  uint64_t a_addr;
  uint64_t b_addr;
  uint64_t c_addr;
  uint64_t ntt_tw_addr;
  uint64_t intt_tw_addr;
  uint64_t zeta_pos_addr;
} kernel_arg_t;

static const uint16_t NTT_TWIDDLES[127] = {
    1729, 2580, 3289, 2642,  630, 1897,  848, 1062,
    1919,  193,  797, 2786, 3260,  569, 1746,  296,
    2447, 1339, 1476, 3046,   56, 2240, 1333, 1426,
    2094,  535, 2882, 2393, 2879, 1974,  821,  289,
     331, 3253, 1756, 1197, 2304, 2277, 2055,  650,
    1977, 2513,  632, 2865,   33, 1320, 1915, 2319,
    1435,  807,  452, 1438, 2868, 1534, 2402, 2647,
    2617, 1481,  648, 2474, 3110, 1227,  910,   17,
    2761,  583, 2649, 1637,  723, 2288, 1100, 1409,
    2662, 3281,  233,  756, 2156, 3015, 3050, 1703,
    1651, 2789, 1789, 1847,  952, 1461, 2687,  939,
    2308, 2437, 2388,  733, 2337,  268,  641, 1584,
    2298, 2037, 3220,  375, 2549, 2090, 1645, 1063,
     319, 2773,  757, 2099,  561, 2466, 2594, 2804,
    1092,  403, 1026, 1143, 2150, 2775,  886, 1722,
    1212, 1874, 1029, 2110, 2935,  885, 2154
};

static const uint16_t INTT_TWIDDLES[127] = {
    1175, 2444,  394, 1219, 2300, 1455, 2117, 1607,
    2443,  554, 1179, 2186, 2303, 2926, 2237,  525,
     735,  863, 2768, 1230, 2572,  556, 3010, 2266,
    1684, 1239,  780, 2954,  109, 1292, 1031, 1745,
    2688, 3061,  992, 2596,  941,  892, 1021, 2390,
     642, 1868, 2377, 1482, 1540,  540, 1678, 1626,
     279,  314, 1173, 2573, 3096,   48,  667, 1920,
    2229, 1041, 2606, 1692,  680, 2746,  568, 3312,
    2419, 2102,  219,  855, 2681, 1848,  712,  682,
     927, 1795,  461, 1891, 2877, 2522, 1894, 1010,
    1414, 2009, 3296,  464, 2697,  816, 1352, 2679,
    1274, 1052, 1025, 2132, 1573,   76, 2998, 3040,
    2508, 1355,  450,  936,  447, 2794, 1235, 1903,
    1996, 1089, 3273,  283, 1853, 1990,  882, 3033,
    1583, 2760,   69,  543, 2532, 3136, 1410, 2267,
    2481, 1432, 2699,  687,   40,  749, 1600
};

// ZETA_POS[k] = 17^{brv7(64+k)} mod 3329 for k = 0..63
// Used by basemul: zeta for pair i = ZETA_POS[i>>1] if i even,
//                                     Q - ZETA_POS[i>>1] if i odd
static const uint16_t ZETA_POS[64] = {
      17, 2761,  583, 2649, 1637,  723, 2288, 1100,
    1409, 2662, 3281,  233,  756, 2156, 3015, 3050,
    1703, 1651, 2789, 1789, 1847,  952, 1461, 2687,
     939, 2308, 2437, 2388,  733, 2337,  268,  641,
    1584, 2298, 2037, 3220,  375, 2549, 2090, 1645,
    1063,  319, 2773,  757, 2099,  561, 2466, 2594,
    2804, 1092,  403, 1026, 1143, 2150, 2775,  886,
    1722, 1212, 1874, 1029, 2110, 2935,  885, 2154
};

static const int NTT_LAYER_OFFSETS[7]  = { 0, 1, 3, 7, 15, 31, 63 };
static const int NTT_LAYER_COUNTS[7]   = { 1, 2, 4, 8, 16, 32, 64 };
static const int INTT_LAYER_OFFSETS[7] = { 0, 64, 96, 112, 120, 124, 126 };
static const int INTT_LAYER_COUNTS[7]  = { 64, 32, 16, 8, 4, 2, 1 };

#endif

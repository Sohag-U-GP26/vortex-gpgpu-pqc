...
// CT Butterfly: rs1=A, rs2=B, rs3=ζ → rd={4'b0, A'[11:0], 4'b0, B'[11:0]}
// Kernel: A' = (rd>>16)&0xFFF, B' = rd&0xFFF
inline __attribute__((const)) int vx_ct_butterfly(int a, int b, int w) {
    int ret;
    __asm__ volatile (".insn r4 %1, 0, 0, %0, %2, %3, %4" : "=r"(ret) : "i"(RISCV_CUSTOM0), "r"(a), "r"(b), "r"(w));
    return ret;
}

// GS Butterfly: rs1=A, rs2=B, rs3=ζ → rd={4'b0, A'[11:0], 4'b0, B'[11:0]}
// Kernel: A' = (rd>>16)&0xFFF, B' = rd&0xFFF
inline __attribute__((const)) int vx_gs_butterfly(int a, int b, int w) {
    int ret;
    __asm__ volatile (".insn r4 %1, 0, 1, %0, %2, %3, %4" : "=r"(ret) : "i"(RISCV_CUSTOM0), "r"(a), "r"(b), "r"(w));
    return ret;
}

// BASEMUL: rs1={B1,A1}, rs2={B0,A0}, rs3=ζ → rd={4'b0, C1[11:0], 4'b0, C0[11:0]}
inline __attribute__((const)) int vx_basemul(int a, int b, int zeta) {
    int ret;
    __asm__ volatile (".insn r4 %1, 0, 2, %0, %2, %3, %4" : "=r"(ret) : "i"(RISCV_CUSTOM0), "r"(a), "r"(b), "r"(zeta));
    return ret;
}

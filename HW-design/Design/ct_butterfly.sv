...
module ct_butterfly (
    ...
    // Extension with 4 zeros prevents interpreting the positive 12 bits as negative
    // (To prevent MSB=1 if bit11 is high), ensuring correct signed calculations
    assign sum_raw  = $signed({4'b0, A_reg}) + $signed({4'b0, BW_mod});
    assign diff_raw = $signed({4'b0, A_reg}) - $signed({4'b0, BW_mod});

    // master valid_out register
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            valid_out <= 1'b0;
        else

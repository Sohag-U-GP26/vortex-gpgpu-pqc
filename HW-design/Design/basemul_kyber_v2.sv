...
//  Version 2: Single Wide Barrett at the end
// ============================================================
//
//  Difference from v1:
//    v1 -> uses 5 instances of barrett_reduction_kyber (24-bit) chained
//    v2 -> accumulates raw products without reduction, then calls
//          barrett_reduction_kyber_wide (36-bit) once per output
//
//  Math (without intermediate reduction):
...
//    Cycle 1 (FF)   : saves products + zeta
//    Cycle 1 (comb) : m11_zeta = m11_r * zeta_r  (36-bit)
//                     raw_C1   = m01_r + m10_r   (25-bit)
//    Cycle 2 (FF)   : saves m00_r2, m11_zeta_r, raw_C1_r
...
module basemul_kyber_v2 #(
    ...
    output logic [11:0] C0,         // C[2i]   — valid after 3 cycles
    output logic [11:0] C1,         // C[2i+1]
    output logic        valid_out
);

    // =========================================================================
    //  Details of Zeta Matrix (Twisting Factors) used in Basecase
    // =========================================================================
    //  In Kyber algorithm, the prime modulus (q) = 3329.
    //  Primitive root ζ (Primitive 512th root of unity) = 17.
    //  In the Basecase stage (last stage of NTT), we use specific ζ values
    //  for each pair of adjacent elements.
    //
    //  The 64 positive ζ values are calculated as follows:
    //      zeta_pos[k] = 17^{br7(64+k)} mod 3329    for each k from 0 to 63
    //  where br7 is the bit reversal of 7 bits.
    //
    //  Values of ZETA_POS matrix (64 positive values):
    ...
    //
    //  How to choose ζ value for each pair (pair_idx from 0 to 127):
    //  - Every two consecutive pairs use the same ZETA_POS but with opposite signs.
    ...
    //
    //  This module (basemul_kyber_v2) receives the pre-calculated zeta value
    //  as a direct input instead of calculating it internally.
    // =========================================================================

    // ----------------------------------------------------------
    // Cycle 0: 4 parallel multipliers 12x12->24 bit
    // ----------------------------------------------------------
    ...
    //   valid_sr[1]: after Cycle-2 FF -> feeds Barrett instances
    // ----------------------------------------------------------
    logic [1:0] valid_sr;
    ...
    // ----------------------------------------------------------
    // Cycle 1 FF: save Cycle-0 products and zeta
    // ----------------------------------------------------------
    ...
    // ----------------------------------------------------------
    // Cycle 2 FF: save Cycle-1 results
    // ----------------------------------------------------------
    logic [23:0] m00_r2;
    ...
    // raw_C1 extended to 36-bit (was only 25-bit, zero-extended)
    ...
    //
    //   valid_sr[1] -> valid_in for Barrett instances
    //   Barrett registers output in internal FF -> Cycle 3
    // ----------------------------------------------------------
    ...
    barrett_reduction_kyber_wide #(.Q(Q)) br_c0 (
        .clk      (clk),
        .rst_n    (rst_n),
        .valid_in (valid_sr[1]),   // Cycle-2 data ready
        .C        (raw_C0),        // 36-bit raw
        .P        (C0),
    ...
    barrett_reduction_kyber_wide #(.Q(Q)) br_c1 (
        ...
        .rst_n    (rst_n),
        .valid_in (valid_sr[1]),
        .C        (raw_C1_ext),    // 25-bit zero-extended to 36-bit
        .P        (C1),
        .valid_out(br_valid_out_c1)
    );

    // valid_out: both Barrett instances output in the same cycle

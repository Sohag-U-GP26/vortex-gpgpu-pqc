...
module tb_ct_butterfly_simple;
    ...
    initial begin
        ...
        int exp_A_out, exp_B_out, bw;
        int pass_count = 0;

        $display("==================================================");
        $display("   ct_butterfly.sv - Simple Screen Test");
        $display("==================================================");
        ...
        for (int i = 0; i < test_A.size(); i++) begin
            ...
            bw = (test_B[i] * test_W[i]) % Q;
            exp_A_out = (test_A[i] + bw) % Q;
            if (exp_A_out < 0) exp_A_out += Q;
            
            exp_B_out = (test_A[i] - bw) % Q;
            if (exp_B_out < 0) exp_B_out += Q;

            @(posedge clk);
            valid_in = 0;
            
            // ct_butterfly has 2 cycles latency
            @(posedge clk);
            @(posedge clk);
            
            if (A_out == exp_A_out && B_out == exp_B_out) begin
                $display("[PASS] Test %02d: CT(A=%4d, B=%4d, W=%4d) -> A'=%4d, B'=%4d", i+1, test_A[i], test_B[i], test_W[i], A_out, B_out);
                pass_count++;
            end else begin
                $display("[FAIL] Test %02d: CT(A=%4d, B=%4d, W=%4d) -> A'=%4d (Exp:%4d), B'=%4d (Exp:%4d)", i+1, test_A[i], test_B[i], test_W[i], A_out, exp_A_out, B_out, exp_B_out);
            end
        end
